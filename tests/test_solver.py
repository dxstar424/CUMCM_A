import numpy as np
from src.solver import Config, step_1d, solve_1d, _surface_state, reference_water_mass
...
def test_reference_mass_is_invariant_for_uniform_shrinkage():
    c=np.full(16,2.55); assert abs(reference_water_mass(c)-reference_water_mass(np.full(16,2.55)))<1e-14

def test_latent_boundary_beta_is_finite():
    cfg=Config(kind='q2',n=16,dt=30,latent=True,Lv=2.4e6)
    T,C,it,J,q=step_1d(cfg,0,np.full(16,40.),np.full(16,2.),30)
    assert np.isfinite(T).all() and np.isfinite(C).all() and J>=0
