...
def step_1d(cfg: Config, t: float, T: np.ndarray, C: np.ndarray, dt: float):
    ...
    for it in range(80):
        ...
        if n > 1:
            diag[:-1] += gt; diag[1:] += gt
        diag[-1] += area[-1] * gb
        beta = gb / cfg.h if cfg.h else 0.0
        rhs[-1] += area[-1] * gb * Tinf(t + dt) - (area[-1] * beta * cfg.Lv * Jw if cfg.latent else 0.0)
...
def reference_water_mass(C, C0=2.55, R0=0.02, L0=0.25):
    """Water mass proxy using fixed material-coordinate dry-mass weights.

    C is dry-basis moisture; the reference ring weights are fixed, so this
    diagnostic remains invariant under a no-flux moving-boundary deformation.
    It is deliberately separate from current-volume averaging.
    """
    C=np.asarray(C); n=C.shape[-1]; xi=np.arange(n+1,dtype=float)/n
    w=np.pi*R0*R0*L0*(xi[1:]**2-xi[:-1]**2)
